...
MXL608_STATUS MxLWare608_API_CfgDevSoftReset(UINT32_608 Port, UINT8_608 devId)
{
  ...
  status = MxLWare608_OEM_WriteRegister(Port, devId, AIC_RESET_REG, 0x00); 
...
MXL608_STATUS MxLWare608_API_CfgDevOverwriteDefaults(UINT32_608 Port, UINT8_608 devId, 
                                                  MXL608_BOOL singleSupply_3_3V)
{
  ...
  status |= MxL608_Ctrl_ProgramRegisters(Port, devId, MxL608_OverwriteDefaults);

  status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x00, 0x01);
  status |= MxLWare608_OEM_ReadRegister(Port, devId, 0x31, &readData);
  readData &= 0x2F;
  readData |= 0xD0;
  status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x31, readData);
  status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x00, 0x00);


  /* If Single supply 3.3v is used */
  if (MXL608_ENABLE == singleSupply_3_3V)
    status |= MxLWare608_OEM_WriteRegister(Port, devId, MAIN_REG_AMP, 0x04);
...
MXL608_STATUS MxLWare608_API_CfgDevXtal(UINT32_608 Port, UINT8_608 devId, MXL608_XTAL_SET_CFG_T xtalCfg)
{
  UINT8_608 status = MXL608_SUCCESS;
  UINT8_608 control = 0;
  ...
  if ((xtalCfg.xtalFreqSel == MXL608_XTAL_16MHz) || (xtalCfg.xtalFreqSel == MXL608_XTAL_24MHz))
  {
    ...
    status = MxLWare608_OEM_WriteRegister(Port, devId, XTAL_CAP_CTRL_REG, control);
    ...
    if (xtalCfg.XtalSharingMode == MXL608_ENABLE) 
    {
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, XTAL_ENABLE_DIV_REG, control);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, XTAL_EXT_BIAS_REG, 0x80);
    }
    else 
    {
      control &= 0x01;
      // program Clock out div & Xtal sharing
      status |= MxLWare608_OEM_WriteRegister(Port, devId, XTAL_ENABLE_DIV_REG, control); 
      status |= MxLWare608_OEM_WriteRegister(Port, devId, XTAL_EXT_BIAS_REG, 0x0A);
    }

    // Main regulator re-program
    if (MXL608_ENABLE == xtalCfg.singleSupply_3_3V)
      status |= MxLWare608_OEM_WriteRegister(Port, devId, MAIN_REG_AMP, 0x14);
  }
  else 
...
MXL608_STATUS MxLWare608_API_CfgDevPowerMode(UINT32_608 Port,
                                          UINT8_608 devId, 
                                          ...
                                          UINT8_608 standbyLt)
{
  ...
  switch(powerMode)
  {
    ...
    case MXL608_PWR_MODE_ACTIVE:
      status |= MxLWare608_OEM_WriteRegister(Port, devId, TUNER_ENABLE_REG, MXL608_ENABLE);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, START_TUNE_REG, MXL608_ENABLE);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x01);
      if (enableLoopthrough == MXL608_ENABLE)     
        status |= MxLWare608_OEM_WriteRegister(Port, devId,DFE_SEQ_TUNE_RF1_BO_REG,0x0E);
      else 
        status |= MxLWare608_OEM_WriteRegister(Port, devId,DFE_SEQ_TUNE_RF1_BO_REG,0x37);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x00); 
      break;

    case MXL608_PWR_MODE_STANDBY:
      status |= MxLWare608_OEM_WriteRegister(Port, devId, START_TUNE_REG, MXL608_DISABLE);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, TUNER_ENABLE_REG, MXL608_DISABLE);
      if ((standbyLt != 0) && (enableLoopthrough == MXL608_ENABLE))
      {
        status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x01);
        status |= MxLWare608_OEM_WriteRegister(Port, devId,DFE_SEQ_TUNE_RF1_BO_REG,(standbyLt & 0x3F));
        status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x00);
      }	
      break;
...
MXL608_STATUS MxLWare608_API_CfgDevGPO(UINT32_608 Port, UINT8_608 devId, MXL608_GPO_STATE_E gpoState)
{
  UINT8_608 status = MXL608_SUCCESS;
  UINT8_608 regData = 0;
  ...
  switch(gpoState)
  {
    ...
    case MXL608_GPO_LOW:
      status = MxLWare608_OEM_ReadRegister(Port, devId, GPO_SETTING_REG, &regData);
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, GPO_SETTING_REG, regData);
      break;
...
MXL608_STATUS MxLWare608_API_ReqDevVersionInfo(UINT32_608 Port, UINT8_608 devId, 
                                            MXL608_VER_INFO_T* mxlDevVerInfoPtr)
{
  ...
  if (mxlDevVerInfoPtr)
  {
    status |= MxLWare608_OEM_ReadRegister(Port, devId, CHIP_ID_REQ_REG, &readBack);
    mxlDevVerInfoPtr->chipId = (readBack & 0xFF); 

    status |= MxLWare608_OEM_ReadRegister(Port, devId, CHIP_VERSION_REQ_REG, &readBack);
...
MXL608_STATUS MxLWare608_API_ReqDevGPOStatus(UINT32_608 Port,
                                          UINT8_608 devId, 
...
MXL608_STATUS MxLWare608_API_ReqDevPllState(UINT32_608 Port, UINT8_608 devId, MXL608_PLL_STATE_E* PllStatePtr)
{
  ...
  *PllStatePtr = MXL608_PLL_STATE_NA; 
  ...
  for (k = 0; k < 7; k++)
  {
    ...
    status |= MxLWare608_OEM_ReadRegister(Port, devId, regAddr[k], &regData[k]);
  }
...
MXL608_STATUS MxLWare608_API_CfgTunerMode(UINT32_608 Port, UINT8_608 devId, 
                                       MXL608_TUNER_MODE_CFG_T tunerModeCfg)
{
  ...
  switch(tunerModeCfg.signalMode)
  {
    ...
    case MXL608_DIG_J83B:
      ...
      status = MxL608_Ctrl_ProgramRegisters(Port, devId, tmpRegTable);

      if (tunerModeCfg.ifOutFreqinKHz < HIGH_IF_35250_KHZ)
      {
        // Low power
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_0, 0xFE);
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_1, 0x10);
      }
      else
      {
        // High power
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_0, 0xD9);
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_1, 0x16);
      }
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, DFE_CSF_SS_SEL, dfeRegData);
    ...
    case MXL608_DIG_ISDBT_ATSC:
      ...
      {
        // Low power
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_0, 0xF9);
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_1, 0x18);
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_PWR, 0xF1);
      }
      else
      {
        // High power
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_0, 0xD9);
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_CFG_1, 0x16);
        status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_IF_PWR, 0xB1);
      }
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, DFE_DACIF_GAIN, dfeRegData);
  ...
  {
    // XTAL calibration
    status |= MxLWare608_OEM_WriteRegister(Port, devId, XTAL_CALI_SET_REG, 0x00);   
    status |= MxLWare608_OEM_WriteRegister(Port, devId, XTAL_CALI_SET_REG, 0x01);   
...
MXL608_STATUS MxLWare608_API_CfgTunerAGC(UINT32_608 Port, UINT8_608 devId, MXL608_AGC_CFG_T agcCfg)
{
  UINT8_608 status = MXL608_SUCCESS;
  UINT8_608 regData = 0; 
      ...
      (agcCfg.agcType <= MXL608_AGC_EXTERNAL))
  {
    ...
    status |= MxLWare608_OEM_ReadRegister(Port, devId, AGC_CONFIG_REG, &regData); 
    regData &= 0xF2; // Clear bits <3:2> & <0>
    regData = (UINT8_608) (regData | (agcCfg.agcType << 2) | 0x01);
    status |= MxLWare608_OEM_WriteRegister(Port, devId, AGC_CONFIG_REG, regData);

    // AGC set point <6:0>
    status |= MxLWare608_OEM_ReadRegister(Port, devId, AGC_SET_POINT_REG, &regData);
    regData &= 0x80; // Clear bit <6:0>
    regData |= agcCfg.setPoint;
    status |= MxLWare608_OEM_WriteRegister(Port, devId, AGC_SET_POINT_REG, regData);

    // AGC Polarity <4>
    status |= MxLWare608_OEM_ReadRegister(Port, devId, AGC_FLIP_REG, &regData);
    regData &= 0xEF; // Clear bit <4>
    regData |= (agcCfg.agcPolarityInverstion << 4);
    status |= MxLWare608_OEM_WriteRegister(Port, devId, AGC_FLIP_REG, regData);
  }
  else
    status = MXL608_INVALID_PARAMETER;
...
MXL608_STATUS MxLWare608_API_CfgTunerLoopThrough(UINT32_608 Port, UINT8_608 devId, MXL608_BOOL loopThroughCtrl, UINT8_608* gainArray, UINT8_608* attArray)
{
  UINT8_608 status = MXL608_SUCCESS;
  UINT8_608 regData;
  ...
  if (loopThroughCtrl <= MXL608_ENABLE)
  {
    ...
    status |= MxLWare608_OEM_ReadRegister(Port, devId, DIG_ANA_GINJO_LT_REG, &regData);

    if (loopThroughCtrl == MXL608_ENABLE)
      regData |= 0x10;  // Bit<4> = 1       
    else
      regData &= 0xEF;  // Bit<4> = 0  
    status |= MxLWare608_OEM_WriteRegister(Port, devId, DIG_ANA_GINJO_LT_REG, regData);
    ...
    if ((loopThroughCtrl == MXL608_ENABLE) && (gainArray != NULL) && (attArray != NULL))
    {
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, DFE_SEQ_DIGANA_LT_GAIN_REG, regData);
      regData = (attArray[3]<<6 ) | (attArray[2] <<4 ) | (attArray[1] <<2 ) | attArray[0] ;
      status |= MxLWare608_OEM_WriteRegister(Port, devId, DFE_SEQ_DIGANA_LT_ATTN_REG, regData);
    }
    status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x00);
  }
  else
...
MXL608_STATUS MxLWare608_API_CfgTunerSouthAfricaLT(UINT32_608 Port, UINT8_608 devId)
{
  UINT8_608 status = MXL608_SUCCESS;
  ...
  status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x01);
  status |= MxLWare608_OEM_WriteRegister(Port, devId, DFE_SEQ_DIGANA_LT_GAIN_REG, 0x95);
  status |= MxLWare608_OEM_WriteRegister(Port, devId, DFE_SEQ_DIGANA_LT_ATTN_REG, 0x00);
  status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x00);
...
MXL608_STATUS MxLWare608_API_CfgTunerChanTune(UINT32_608 Port, UINT8_608 devId, 
                                           MXL608_CHAN_TUNE_CFG_T chanTuneCfg)
{
  ...
  // Abort Tune
  status |= MxLWare608_OEM_WriteRegister(Port, devId, START_TUNE_REG, 0x00); 
  ...
  if (chanTuneCfg.startTune == MXL608_ENABLE)
  {
    if (chanTuneCfg.signalMode <= MXL608_DIG_J83B) 
    {
      ...
      {
        status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x7C, 0x1F);
      ...
      {
        status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x7C, 0x9F);
        if ((chanTuneCfg.signalMode == MXL608_DIG_DVB_C) || (chanTuneCfg.signalMode == MXL608_DIG_J83B)) 
          regData = 0xD1;
        else
        regData = 0x91;       
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x00, 0x01);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x31, regData);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x00, 0x00);
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, FINE_TUNE_INIT1_REG, dfeAgcRssispData); 
#endif
      ...
      if (freqLutPtr)
        status |= Ctrl608_SetRfFreqLutTblReg(Port, devId, chanTuneCfg.freqInHz, freqLutPtr);
      // Bandwidth <7:0>
      switch(chanTuneCfg.bandWidth)
      {
        ...
        case MXL608_TERR_BW_8MHz:
            status |= MxLWare608_OEM_WriteRegister(Port, devId, CHAN_TUNE_BW_REG, (UINT8_608)chanTuneCfg.bandWidth);
            ...
            // Set RF  
            status |= MxLWare608_OEM_WriteRegister(Port, devId, CHAN_TUNE_LOW_REG, (UINT8_608)(freq & 0xFF));
            status |= MxLWare608_OEM_WriteRegister(Port, devId, CHAN_TUNE_HI_REG, (UINT8_608)((freq >> 8 ) & 0xFF));
            break;
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, TUNER_ENABLE_REG, 0x01);
      ...
      status |= MxLWare608_OEM_ReadRegister(Port, devId, 0xB6, &agcData);
      status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x01); 
      status |= MxLWare608_OEM_ReadRegister(Port, devId, 0x60, &dfeTuneData);
      status |= MxLWare608_OEM_ReadRegister(Port, devId, 0x5F, &dfeCdcData);
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x60, dfeTuneData); 
      status |= MxLWare608_OEM_WriteRegister(Port, devId, 0x5F, dfeCdcData); 
      status |= MxLWare608_OEM_WriteRegister(Port, devId, PAGE_CHANGE_REG, 0x00); 
      status |= MxLWare608_OEM_WriteRegister(Port, devId, 0xB6, agcData); 
      
      // Bit <0> 1 : start , 0 : abort calibrations
      status |= MxLWare608_OEM_WriteRegister(Port, devId, START_TUNE_REG, 0x01); 
...
MXL608_STATUS MxLWare608_API_CfgTunerIFOutParam(UINT32_608 Port, UINT8_608 devId, MXL608_IF_OUT_CFG_T ifOutCfg)
{
  UINT16_608 ifFcw;
  UINT8_608 status = MXL608_SUCCESS;
  UINT8_608 readData = 0;
  ...
  status = MxLWare608_OEM_ReadRegister(Port, devId, IF_FREQ_SEL_REG, &readData);
  ...
  if (ifOutCfg.manualFreqSet == MXL608_ENABLE)
  {
    ...
    readData |= 0x20;
    status = MxLWare608_OEM_WriteRegister(Port, devId, IF_FREQ_SEL_REG, readData);

    // Manual IF freq set
    ifFcw = (UINT16_608)(ifOutCfg.manualIFOutFreqInKHz * 8192 / 216000);
    control = (ifFcw & 0xFF); // Get low 8 bit 
    status |= MxLWare608_OEM_WriteRegister(Port, devId, IF_FCW_LOW_REG, control); 

    control = ((ifFcw >> 8) & 0x0F); // Get high 4 bit 
    status |= MxLWare608_OEM_WriteRegister(Port, devId, IF_FCW_HIGH_REG, control);
  }
  ...
  if (status == MXL608_SUCCESS)
  {
    ...
    if (ifOutCfg.ifInversion <= MXL608_ENABLE)
    {
      ...
      status |= MxLWare608_OEM_WriteRegister(Port, devId, IF_PATH_GAIN_REG, control);
    } 
    else
      status |= MXL608_INVALID_PARAMETER;
  }
...
MXL608_STATUS MxLWare608_API_ReqTunerAGCLock(UINT32_608 Port, UINT8_608 devId, MXL608_BOOL* agcLockStatusPtr)
{
  MXL608_STATUS status = MXL608_SUCCESS;
  UINT8_608 regData = 0;
  ...
  if (agcLockStatusPtr)
  {
    status = MxLWare608_OEM_ReadRegister(Port, devId, AGC_SAGCLOCK_STATUS_REG, &regData);  
...
MXL608_STATUS MxLWare608_API_ReqTunerLockStatus(UINT32_608 Port, UINT8_608 devId, MXL608_BOOL* rfLockPtr, 
                                                          MXL608_BOOL* refLockPtr)
{
  ...
  {
    status = MxLWare608_OEM_ReadRegister(Port, devId, RF_REF_STATUS_REG, &regData);  
...
MXL608_STATUS MxLWare608_API_ReqTunerRxPower(UINT32_608 Port, UINT8_608 devId, SINT16* rxPwrPtr)
{
  UINT8_608 status = MXL608_SUCCESS;
  UINT8_608 regData = 0;
  ...
  if (rxPwrPtr)
  {
    ...
    status = MxLWare608_OEM_ReadRegister(Port, devId, RFPIN_RB_LOW_REG, &regData);
    tmpData = regData;

    // RF input power high <1:0>
    status |= MxLWare608_OEM_ReadRegister(Port, devId, RFPIN_RB_HIGH_REG, &regData);
