...
MXL608_STATUS MxL608_Ctrl_ProgramRegisters(UINT32_608 Port, UINT8_608 devId, PMXL608_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  MXL608_STATUS status = MXL608_TRUE;
  UINT16_608 i = 0;
  UINT8_608 tmp = 0;
  
  while (status == MXL608_TRUE)
  {
    ...
    {
      status = MxLWare608_OEM_ReadRegister(Port, devId,ctrlRegInfoPtr[i].regAddr, &tmp);
      if (status != MXL608_TRUE) break;;
    }
    
    tmp &= (UINT8_608) ~ctrlRegInfoPtr[i].mask;
    tmp |= (UINT8_608) ctrlRegInfoPtr[i].data;

    status = MxLWare608_OEM_WriteRegister(Port, devId,ctrlRegInfoPtr[i].regAddr, tmp);
    if (status != MXL608_TRUE) break;
...
MXL608_STATUS MxL608_Ctrl_WriteRegField(UINT32_608 Port, UINT8_608 devId, PMXL608_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  MXL608_STATUS status = MXL608_TRUE;
  UINT8_608 tmp = 0;
  ...
  {
      status = MxLWare608_OEM_ReadRegister(Port, devId, ctrlRegInfoPtr->regAddr, &tmp);
      if (status != MXL608_TRUE) return status;
   }

  tmp &= (UINT8_608)~ctrlRegInfoPtr->mask;  // Clear the field that need to set value 
  tmp |= (UINT8_608)(ctrlRegInfoPtr->data & ctrlRegInfoPtr->mask); 

  status = MxLWare608_OEM_WriteRegister(Port, devId, ctrlRegInfoPtr->regAddr, tmp);

  return status;
}

/*------------------------------------------------------------------------------
--| FUNCTION NAME : Ctrl608_SetRfFreqLutTblReg
--|
--| AUTHOR        : Dong Liu
--|
...
MXL608_STATUS Ctrl608_SetRfFreqLutTblReg(UINT32_608 Port, UINT8_608 devId, UINT32_608 FreqInHz, PMXL608_CHAN_DEPENDENT_FREQ_TABLE_T freqLutPtr)
{
  UINT8_608 status = MXL608_TRUE;
  UINT8_608 idx = 0;
  UINT8_608 regSetData[MXL608_MAX_SPUR_REG_NUM] = {0, 0};
  ...
  for (idx = 0; idx < MxL608_SPUR_REGISTER.SpurRegNum; idx++)
    status |= MxLWare608_OEM_WriteRegister(Port, devId, MxL608_SPUR_REGISTER.SpurRegAddr[idx], regSetData[idx]);
