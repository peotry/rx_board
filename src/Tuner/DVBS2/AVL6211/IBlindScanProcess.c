...
AVL_DVBSx_ErrorCode AVL_DVBSx_IBlindScanProcess_LockChannel(AVL_uint32 u32Port, struct AVL_DVBSx_Chip  *pAVL_DVBSxChip,struct AVL_Tuner *pAVL_Tuner,AVL_uint32 Freq_MHz, AVL_uint32 SymbolRate_Hz)
{
    ...
    r = pTuner->m_pLockFunc(u32Port, Freq_MHz, SymbolRate_Hz, pAVLChip);
    if(AVL_DVBSx_EC_OK != r)
    {
        return (r);
    }
    ...
    r = AVL_DVBSx_IRx_LockChannel(u32Port, &Channel, pAVLChip);
    if (AVL_DVBSx_EC_OK != r)
    {
        return (r);
    }
    ...
    do
    {
        ...
        r = AVL_DVBSx_IRx_GetLockStatus(u32Port, &uiLockStatus, pAVLChip);
...
AVL_DVBSx_ErrorCode AVL_DVBSx_IBlindScanProcess_ReadChannels(AVL_uint32 u32Port, struct AVL_DVBSx_Chip * pAVLChip,struct AVL_DVBSx_BlindScanProcessData *bsData,AVL_uint16 Channels_Count)
{
    AVL_DVBSx_ErrorCode r = AVL_DVBSx_EC_OK;
    ...
    while(Channels_Count)
    {
        ...
        r = AVL_DVBSx_IBlindScan_ReadChannelInfo( u32Port, start_index, &read_count, channels_Temp, pAVLChip);
...
AVL_DVBSx_ErrorCode AVL_DVBSx_IBlindScanProcess_ScanStart(AVL_uint32 u32Port, struct AVL_DVBSx_Chip * pAVLChip, struct AVL_Tuner * pTuner,struct AVL_DVBSx_BlindScanProcessData *bsData)
{
    AVL_DVBSx_ErrorCode r = AVL_DVBSx_EC_OK;
    struct AVL_DVBSx_BlindScanPara bsPara;
    AVL_uint16 Start_Freq_100KHz;   

    r = AVL_DVBSx_IBase_SetFunctionalMode(u32Port, pAVLChip, AVL_DVBSx_FunctMode_BlindScan); 
    ...
    r |= AVL_DVBSx_IBlindScan_Reset( u32Port, pAVLChip);
    ...
    r = pTuner->m_pLockFunc(u32Port, Start_Freq_100KHz, pTuner->m_uiSymbolRate_Hz, pAVLChip);   //Lock the tuner.
    ...
    r |= AVL_DVBSx_IBlindScan_Scan( u32Port, &bsPara,bsData->m_uiTuner_MaximumLPF_100KHz, pAVLChip);

    return r;
}

//Prepare and start the blind scan process
AVL_DVBSx_ErrorCode AVL_DVBSx_IBlindScanProcess_Start(AVL_uint32 u32Port, struct AVL_DVBSx_Chip * pAVLChip, struct AVL_Tuner * pTuner,struct AVL_DVBSx_BlindScanProcessData *bsData, struct AVL_DVBSx_BlindScanPara bsPara, AVL_uint16 uiTuner_MaximumLPF_100KHz,AVL_uint16 uiTuner_FlatLPF_100KHz)
{
    AVL_DVBSx_ErrorCode r = AVL_DVBSx_EC_OK;
    AVL_int16 i;
    ...
    r = AVL_DVBSx_IBlindScanProcess_ScanStart(u32Port, pAVLChip, pTuner, bsData);

    return r;
}

//Update the blind scan progress and continue the blind scan process
enum AVL_DVBSx_BlindscanProcessStatus AVL_DVBSx_IBlindScanProcess_ScanLock(AVL_uint32 u32Port, struct AVL_DVBSx_Chip * pAVLChip, struct AVL_Tuner * pTuner,struct AVL_DVBSx_BlindScanProcessData *bsData,AVL_uint16 *bsProgress)
{
    AVL_DVBSx_ErrorCode r = AVL_DVBSx_EC_OK;
    ...
    {
        r = AVL_DVBSx_IBlindScan_GetScanStatus(u32Port, &bsInfo, pAVLChip);  //Query the internal blind scan procedure information.
        if(AVL_DVBSx_EC_OK == r)
        {
            ...
            if(bsInfo.m_uiProgress == 100)
            {
                ...
                {
                    r = AVL_DVBSx_IBlindScanProcess_ReadChannels(u32Port, pAVLChip, bsData,bsInfo.m_uiChannelCount); 
                }
    ...
    if(bsData->m_eBSStatusFlag == AVL_DVBSx_BS_Status_Locked)    //Locked status, there are no channels to lock goto next scan
    {
        ...
        {
            r = AVL_DVBSx_IBase_SetFunctionalMode(u32Port, pAVLChip, AVL_DVBSx_FunctMode_Demod); 
            r |= AVL_DVBSx_IBlindScanProcess_LockChannel(u32Port, pAVLChip,pTuner, bsData->channel_buf[bsData->m_uiLockedChannelIndex].m_uiFrequency_kHz/1000, bsData->channel_buf[bsData->m_uiLockedChannelIndex].m_uiSymbolRate_Hz);
            if(r == AVL_DVBSx_EC_OK)
            {
...
AVL_DVBSx_ErrorCode AVL_DVBSx_IBlindScanProcess_Stop(AVL_uint32 u32Port, struct AVL_DVBSx_Chip * pAVLChip, struct AVL_DVBSx_BlindScanProcessData *bsData)
{
    AVL_DVBSx_ErrorCode r = AVL_DVBSx_EC_OK;
    if(bsData == AVL_NULL)
    {
