...
{
    srv_shutdown = ((true == isShutDown) ? 1 : 0); 
    graceful_shutdown = ((true == isShutDown) ? 1 : 0);
...
{
    return ((1 == g_ServerFinished) ? true : false);
}
...
int lighttpd_configuration(int argc, char **argv)
{
    ...
    //if (config_read(srv, "/tmp/lighttpd-1.4.33-arm/config/lighttpd.conf")) {
    if (config_read(srv, CONFIG_FILE)) {
        server_free(srv);
        return -1;
    }
    //特殊代码，防止 plugins_call_set_defaults 函数调用后，修改size的值，导致退出服务器，释放数据时，释放到非法内存，导致程序崩溃
    g_TouchedSize = srv->config_touched->size - 1;
    ...
    //特殊代码，防止 plugins_call_set_defaults 函数调用后，修改size的值，导致退出服务器，释放数据时，释放到非法内存，导致程序崩溃
    srv->config_touched->size = g_TouchedSize;
    server_free(srv);
