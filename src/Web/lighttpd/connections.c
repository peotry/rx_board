 ...
 *****************************************************************************************/
    ...
    {
         buffer        * g      = NULL;
         wvErrCode     eRetCode = WV_SUCCESS;
         
         //清空已有数据
         chunkqueue_reset(con->write_queue);

         //获取首地址
         g = chunkqueue_get_append_buffer(con->write_queue);

         //默认为返回成功
         con->http_status = 200;    

         //WEB 数据的组装
         eRetCode = web_ProcessData(g ,srv, con);           
         if (WV_SUCCESS != eRetCode)       
         {
            //请求不存在
            if (WV_ERR_WEB_NOTEXIST == eRetCode)
            {
                con->http_status = 404;
            }
         }
...
found_header_end:
    ...
    case CON_STATE_READ_POST:
        for (c = cq->first; c && (dst_cq->bytes_in != (off_t)con->request.content_length); c = c->next) {
            ...
            if (true == web_ProcessIsLargeData(con)) {
                //con->request.content_length > WEB_MAX_DATA_LEN && //大小限制先放开
                //不是我们需要的POST请求不让存储数据
                //if (false == web_ProcessIsLargeData(con))
                //{
                ...
                if (dst_cq->last &&
                    ...
                    if (dst_cq->last->file.length < MAX_UPGRADE_FILE_SIZE) {
                        dst_c = dst_cq->last;
