...
#include "AppGlobal.h"
char g_arcUpgradeFiledir[MAX_FILENAME_LENGTH];
...
static void chunk_reset(chunk *c) {
    ...
    //char arcCommand[MAX_COMMAND_LENGTH] = {0};
    
    buffer_reset(c->mem);

    if (c->file.is_temp && !buffer_is_empty(c->file.name)) {
        //unlink(c->file.name->ptr);
        //snprintf( arcCommand, MAX_COMMAND_LENGTH, "mv %s /upload",c->file.name->ptr);
        //system(arcCommand);
    }
...
chunk *chunkqueue_get_append_tempfile(chunkqueue *cq) {
    ...
    memcpy(g_arcUpgradeFiledir,template->ptr,strlen(template->ptr));
