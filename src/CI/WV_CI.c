...
U32                 g_Status_CI_ResetTimes[WVCI_MAX_SLOT_NUM] = {0};                     // CIÍ¨ÐÅÒì³£´ÎÊýÍ³¼Æ
...
DWORD     g_dwNumPCMCIASlots = WVCI_MAX_SLOT_NUM;     // ×î´óÖ§³ÖÍ¨µÀÊýÄ¿£¬Ð­ÒéÕ»ÒªÇó¶¨Òå
...
int WVCI_Setup()
{
   ...
   // WVCI_ReleaseSemphore();
    ...
    // Õý³£Æô¶¯Ê±£¬ÏÈ¹Ø±ÕCAM¹©µç£¬ÒòÎªÄ¿Ç°´ø¼à¿ØµÄÂß¼­ÊÇÄ¬ÈÏ¹©µç
    for (i = 0; i < WVCI_MAX_SLOT_NUM; i++)
    {
 ...
 * DESCRIPTION: - 
 *     µ±Ä£¿é³õÊ¼»¯É¹Ê±,×Ô¶¯ÖØÐÂ·¢ËÍPMTµ½Ä£¿é
 *     ×¢Òâ,²»ÄÜÓëCIMODULEµ×²ãÍ¨ÐÅÍ¬Ò»¸öÈÎÎñ, ÒòÎª²¿·ÖCAMÔÚ·¢ËÍPMT¹ý¿ìÊ±´æÔÚPMT¶ªÊ§ÎÊÌâ...
 * Input  : 
 * Output : 
 * Returns: 
 * 
...
/******************************************************************************
 * WVCI_MessageCallback() - µ×²ã»Øµ÷Ê¹ÓÃ
 * DESCRIPTION: - 
 *     .. 
 * Input  : 
 * Output : 
...
#define STREAM_TYPE_VIDEO2      0x01        //ISO/IEC 13818 ÊÓÆµ
#define STREAM_TYPE_VIDEO1      0x01        //ISO/IEC 11172     ÊÓÆµ
#define STREAM_TYPE_VIDEO       0x02        //ISO/IEC 13818-2   ÊÓÆµ
#define STREAM_TYPE_VIDEO_ATSC  0x80        //ATSC ÊÓÆµ
#define STREAM_TYPE_VIDEO_H264  0x1B        //H264 ÊÓÆµ
#define STREAM_TYPE_VIDEO_MPEG4 0x10        //MPEG4 ÊÓÆµ
#define STREAM_TYPE_VIDEO_AVS   0x42        //AVS ÊÓÆµ
/******************************************************************************
 ...
 * Input  : 
 * Output : 
 * Returns: ÎªÊÓÆµÔò·µ»ØTRUE,·ñÔò·µ»ØFALSE
 * 
 ******************************************************************************/
 ...
 * DESCRIPTION: - 
 *          1. ¸ù¾Ý¹ýCAMµÄÂëÂÊÈ·¶¨PES¼à¿ØÊÇ·ñ¿ªÆô
 *          
 * Input  : u8Slot:                 Ä£¿éÍ¨µÀ
            u8Enable                0: ¹Ø±Õ    1: ¿ªÆô
 * Output : None    
 * Returns: WVCI_SUCCESS                        ²Ù×÷³É¹¦
            WVCI_ERROR_PARAMS                   ²ÎÊý´íÎó
 * 
 ******************************************************************************/
 ...
 * DESCRIPTION: - Èç¹ûµ±Ç°ËÑË÷µÄ½ÚÄ¿Á÷ÊÇ´¦ÓÚPES¼à¿Ø×´Ì¬£¬Ôò°Ñ¸ÃTSÁ÷µÄ¼à¿Ø¹Ø±Õ£¬
 *                ÒÔ·ÀÖ¹Òì³£¸´Î»CAM£¬²¢ÓÉ´Ë´øÀ´µÄÁ÷½ØÖ¹
 *                (CAM³õÊ¼»¯µÄÊ±ºò»áÖÐ¶Ï¹ýCAMµÄÁ÷)
 *     ÔÚÃ¿´ÎÖØÐÂËÑË÷½ÚÄ¿Ê±µ÷ÓÃ
...
int WVCI_PES_Checking(void)
{
    ...
    u8Slot = 0;
    {
        ...
        if (g_PES_ENABLE[u8Slot] != 0x01                    ||      // Èí¼þ¿ØÖÆ¿ª¹Ø
            g_PESCHECK[u8Slot].u8BitrateEnable != 0x01)             // ÂëÂÊ¿ØÖÆ¿ª¹Ø: Ö»ÓÐÁ½¸ö¿ª¹Ø¾ù´ò¿ªÊ±,²Å½øÐÐPES¼ì²â
        {
            g_PESCHECK[u8Slot].u8CheckState = STATE_CHECK_PES_INIT;
            //continue;
            return WVCI_SUCCESS;
            ...
            switch(g_PESCHECK[u8Slot].u8CheckState)
            {
                ...
                case STATE_CHECK_PES_RUNNING:
                {
                    ...
                    { 
                        nRet    = WVCI_PES_ClrStatus(u8Slot);                   
                        g_PESCHECK[u8Slot].u8StateCnt++;                      
                        // 10:ÊÇ¸ö¾­ÑéÖµ:¹ýÐ¡»áµ¼ÖÂÎÞÐÅºÅÊäÈëÊ±,ÎóÅÐÎªÒì³£. ¹ý´ó»áµ¼ÖÂ´¥·¢¸´Î»¹ý¾Ã»ò¸ù±¾²»½â¾ö. Ä¿Ç°×Ô²âÇé¿ö
                        // ²âÊÔ»ÝÖÝÓÐÏßÊµÊ±ÐÅºÅ´¦ÀíÕý³£
                        // CONAX ²âÊÔÁ÷(CD1 ts1.trp)Õý³£
                        // CTI²âÊÔÁ÷(T01_regstbscno48keysepg.ts)´¥·¢²»³É¹¦
...
#ifdef ENABLE_CHECK_DECODER_ESLIST_2DESCRAMBLE                      // Ïã¸Û¿Í»§£¬¾ÉCAM£¬µ¥¸ö½ÚÄ¿Ö»Ö§³Ö10¸öES
#define WVCI_SERVICE_MAX_ES_NUM                     (32)            // µ¥¸ö½ÚÄ¿×î´óÖ§³ÖES¸öÊý
typedef struct 
{
...
static U8 WVCI_GetCAMMaxSupportedESNum(U8 u8Slot)
{
    ...
    {
        // Èç¹û²ÎÊýÖÐÃ»ÓÐÖ¸¶¨CAMµ¥¸ö½ÚÄ¿×î´óÖ§³ÖESÊýÄ¿£¬Ä¬ÈÏ32
...
/******************************************************************************
 * WVCI_GenerateCAPMT() - °´±ê×¼Éú³É½ÚÄ¿µÄCAPMT
 * DESCRIPTION: - 
 *          1. ´¦Àí½ÚÄ¿ÃèÊöÐÅÏ¢
 *          2. ´¦Àí½ÚÄ¿ÏÂËùÓÐÓë½âÈÅÓÐ¹ØµÄESÃèÊöÐÅÏ¢
 *          
 * Input  : u8Slot:                 Ä£¿éÍ¨µÀ
            u16InProgIndex          ½ÚÄ¿Ë÷Òý
            pstParamTS              ÏµÍ³Á÷½á¹¹Ö¸Õë
            eMode                   CAPMTÄ£Ê½(CAPMTLISTMANAGEMENTÀàÐÍ)
 * Output : None    
 * Returns: WVCI_SUCCESS                        ²Ù×÷³É¹¦
            WVCI_FAILURE                        ²Ù×÷Ê§°Ü
 * 
...
int WVCI_GenerateCAPMT(U8 u8Slot, U16 u16OutProgIndex, TSInfo *pstParamTS, U8 eMode)
{
    ...
    {
        // Ö¸¶¨Îª½ö½ö½âÈÅÒ»¸ö½ÚÄ¿£¬Í¬Ê±ºóÃæµÄES ECMÃèÊöÎªÒ»¸öÓë½ÚÄ¿ÎÞ¹ØµÄPID
        // ·ñÔòÈç¹ûÊ¹ÓÃCA_PMT_CMD_ID_NOT_SELECTED²ÎÊý£¬²¿·ÖCAM»¹ÊÇ²»ÄÜÂú×ãÍ£Ö¹½âÈÅ
...
/******************************************************************************
 * WVCI_UpdateSetting() - ¸ù¾ÝTSÅäÖÃ¸üÐÂCIÄ£¿éµÄÓ¦ÓÃ
 * DESCRIPTION: - 
 *          1. ±éÀúËùÓÐ½ÚÄ¿£¬ÅÐ¶ÏÊÇ·ñÉèÖÃ½âÈÅ
 *          2. ¸ù¾ÝÉèÖÃ£¬Éú³É¶ÔÓ¦µÄCAPMT£¬²¢¼ÓÈëµ½Ð­ÒéÕ»µÄ·¢ËÍ¶ÓÁÐ
 *          
 * Input  : emSlotMode:         ÅäÖÃÄ£Ê½£¬¿Éµ¥¶ÀÄ³¸öÍ¨µÀ»òÁ½¸öÍ¨µÀÒ»ÆðÅäÖÃ
 * Output : None    
 * Returns: WVCI_SUCCESS        ²Ù×÷³É¹¦
            WVCI_FAILURE        ²Ù×÷Ê§°Ü
 * 
...
/******************************************************************************
 * TSMUX_CI_SetMaxBitrate() - ÉèÖÃCIÄ£¿éÖ§³ÖµÄ×î´óÂëÂÊ
 * DESCRIPTION:
 * Input  : u32Bitrate:         Ö§³ÖµÄ×î´óÂëÂÊ
 * Output : None    
 * Returns: WVCI_SUCCESS        ²Ù×÷³É¹¦
            WVCI_FAILURE        ²Ù×÷Ê§°Ü
 * 
...
Description:  »ñÈ¡CI½âÈÅ×´Ì¬ÐÅÏ¢
Input:        u8Slot:CIµÄCAM²ÛºÅ
Output:       pstCIStatus:CI½âÈÅ×´Ì¬ÐÅÏ¢
Return:       
